...
import { calcculatePercentage, getChartData, getInventories } from "../utilities/features.js";
...
export const getDashboardStats = TryCatch(async(req, res, next)=>{
    ...
    if(myCache.has(key)) stats = JSON.parse(myCache.get(key) as string)
        ...
        const changePercent = {
            ...
            revenue: calcculatePercentage(thisMonthRevenue, lastMonthRevenue),

            product: calcculatePercentage(thisMonthProducts.length, lastMonthProducts.length),

            user: calcculatePercentage(thisMonthUsers.length, lastMonthUsers.length),

            order: calcculatePercentage(thisMonthOrders.length, lastMonthOrders.length),
        }
